...
public class CustomPipelinePlayer : MonoBehaviour {
	...
	void Start () {
		...
		m_Texture.SetPipeline (pipeline+ " ! video/x-raw,format=I420 ! videoflip method=5 ! videoconvert !  appsink name=videoSink");
    ...
    public void ChangeThePort() {
        ...
        m_Texture.SetPipeline("udpsrc port="+ port.text +" ! application/x-rtp ! rtpjitterbuffer ! rtph264depay ! decodebin  ! video/x-raw,format=I420 ! videoflip method=5 ! videoconvert !  appsink name=videoSink");
        m_Texture.Player.CreateStream();
        m_Texture.Player.Play ();
    }
